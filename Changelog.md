...
from PHD2, and only then download and install the new version. Future versions will not
have this restriction: HelioMaker will handle the update fully automatically.
* Eclipse capture is considerably more resilient around the critical phases
* PHD2 recovery now reconnects a disconnected guide camera or mount before trying to resume
solar guiding, while eclipse imaging continues independently
* HelioMaker can now recover automatically from a confirmed SharpCap capture stall: it sounds
